...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2,
        ...
        @param attention_type (str): Attention scoring type: multiplicative, dot, or additive
        """
        ...
        self.att_enc_proj = None
        self.att_dec_proj = None
        self.att_v = None
        ...
        if attention_type == 'additive':
            self.att_enc_proj = nn.Linear(2 * hidden_size, hidden_size, bias=False)
            self.att_dec_proj = nn.Linear(hidden_size, hidden_size, bias=False)
    ...
    def _prepare_enc_hiddens_for_attention(self, enc_hiddens: torch.Tensor) -> torch.Tensor:
        """Prepare encoder hidden states for attention scoring.

        @param enc_hiddens (Tensor): Encoder hidden states, shape (b, src_len, 2*h).
        @returns enc_hiddens_proj (Tensor): Projected encoder states for attention, shape (b, src_len, h).
        """
        if self.attention_type == 'multiplicative':
            return self.att_projection(enc_hiddens)
        if self.attention_type == 'dot':
            forward, backward = enc_hiddens.chunk(2, dim=-1)
            return forward + backward
        if self.attention_type == 'additive':
            return self.att_enc_proj(enc_hiddens)
        raise ValueError(f'Unknown attention_type: {self.attention_type}')

    def _compute_attention_scores(self, dec_hidden: torch.Tensor,
                                  enc_hiddens_proj: torch.Tensor) -> torch.Tensor:
        """Compute attention scores e_t before masking and softmax.

        @param dec_hidden (Tensor): Decoder hidden state, shape (b, h).
        @param enc_hiddens_proj (Tensor): Prepared encoder states, shape (b, src_len, h).
        @returns e_t (Tensor): Attention scores, shape (b, src_len).
        """
        if self.attention_type in ('multiplicative', 'dot'):
            return torch.bmm(dec_hidden.unsqueeze(1), enc_hiddens_proj.transpose(1, 2)).squeeze(1)
        if self.attention_type == 'additive':
            dec_proj = self.att_dec_proj(dec_hidden).unsqueeze(1)
            return self.att_v(torch.tanh(enc_hiddens_proj + dec_proj)).squeeze(-1)
        raise ValueError(f'Unknown attention_type: {self.attention_type}')
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self._prepare_enc_hiddens_for_attention(enc_hiddens)
        Y = self.model_embeddings.target(target_padded)
        for y in torch.split(Y, 1, dim=0):
            y = y.squeeze(0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = self._compute_attention_scores(dec_hidden, enc_hiddens_proj)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[
        ...
        src_encodings_att_linear = self._prepare_enc_hiddens_for_attention(src_encodings)
    ...
    def load(model_path: str):
        ...
        args.setdefault('attention_type', 'multiplicative')
    ...
    def save(self, path: str):
        ...
        params = {
            'args': dict(embed_size=self.model_embeddings.embed_size, hidden_size=self.hidden_size,
                         dropout_rate=self.dropout_rate, attention_type=self.attention_type),
            'vocab': self.vocab,
