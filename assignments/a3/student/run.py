...
Options:
    ...
    --embed-size=<int>                      embedding size [default: 1024]
    ...
    --attention-type=<str>                  attention scoring type: multiplicative, dot, or additive [default: multiplicative]
    --max-iter=<int>                        stop after this many training iterations (0 = unlimited) [default: 0]
    --metrics-out=<file>                    write final metrics JSON to this path (optional)
"""
import json
import math
import sys
...
def train(args: Dict):
    ...
    if max_iter > 0 or attention_type != 'multiplicative':
        tensorboard_path = f"{tensorboard_path}_{attention_type}"
    ...
    while True:
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if max_iter > 0 and train_iter >= max_iter:
                print('reached max training iterations (%d)' % max_iter, file=sys.stderr)
                last_window_train_loss = report_loss / report_tgt_words if report_tgt_words > 0 else float('nan')
                final_train_loss = cum_loss / cum_tgt_words if cum_tgt_words > 0 else float('nan')
                if report_tgt_words > 0:
                    writer.add_scalar("loss/train", last_window_train_loss, train_iter)
                    writer.add_scalar("perplexity/train", math.exp(last_window_train_loss), train_iter)
                dev_ppl = evaluate_ppl(model, dev_data, batch_size=128)
                writer.add_scalar("loss/val", math.log(dev_ppl), train_iter)
                writer.add_scalar("perplexity/val", dev_ppl, train_iter)
                print('final train loss %.4f, last-window train loss %.4f, dev ppl %.4f' % (
                    final_train_loss, last_window_train_loss, dev_ppl), file=sys.stderr)
                ...
                if args['--metrics-out']:
                    metrics = {
                        ...
                        'last_window_train_loss': last_window_train_loss,
                    ...
                    metrics_path = Path(args['--metrics-out'])
                    metrics_path.parent.mkdir(parents=True, exist_ok=True)
                    with open(metrics_path, 'w') as f:
                        json.dump(metrics, f, indent=2)
                    print('wrote metrics to [%s]' % metrics_path, file=sys.stderr)
                exit(0)
