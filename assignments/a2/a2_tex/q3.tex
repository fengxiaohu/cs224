...
\begin{parts}
    ...
    {\small
    \begin{tabular}{@{}p{4.7cm}|p{5.3cm}|p{3.5cm}|l@{}}
    Stack & Buffer & New dependency & Transition \\ \hline
    [ROOT] & [I, presented, my, findings, at, the, NLP, conference] &  & Initial Configuration \\
    $[$ROOT, I] & [presented, my, findings, at, the, NLP, conference] &  &  \texttt{SHIFT}  \\
    $[$ROOT, I, presented] & [my, findings, at, the, NLP, conference] &  &  \texttt{SHIFT}  \\
    ...
    \ifans{A sentence of \(n\) words is parsed in \(2n\) steps: each word is shifted onto the stack once (\(n\) SHIFT operations) and later attached as a dependent by LEFT-ARC or RIGHT-ARC once (\(n\) arc operations). ROOT starts on the stack and is never shifted or popped as a dependent.}
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \ifans{Let \(\bz=\bx\bW+\bb_1\), so \(h_i=\max(z_i,0)\) and \(z_i=\sum_k x_k W_{ki}+(b_1)_i\).
        Ignoring the undefined point \(z_i=0\), \(\partial h_i/\partial z_i=\mathbf{1}\{z_i>0\}\) and \(\partial z_i/\partial x_j=W_{ji}\). Therefore
        \[
        \frac{\partial h_i}{\partial x_j}
        = \mathbf{1}\{( \bx\bW+\bb_1)_i > 0\}\, W_{ji}.
        \]}
        ...
        \ifans{Since \(\by\) is one-hot at class \(c\), \(CE(\by,\byt)=-\log\hat y_c\). With \(\hat y_k=\mathrm{softmax}(\bl)_k\),
        \(\partial J/\partial \hat y_k=-y_k/\hat y_k\) and \(\partial \hat y_k/\partial l_i=\hat y_k(\delta_{ki}-\hat y_i)\).
        Chaining these derivatives,
        \begin{align*}
        \frac{\partial CE}{\partial l_i}
        &= \sum_{k=1}^{3}\Bigl(-\frac{y_k}{\hat y_k}\Bigr)\hat y_k(\delta_{ki}-\hat y_i)
        = -y_i+\hat y_i\sum_{k=1}^{3}y_k
        = \hat y_i-y_i.
        \end{align*}
        Equivalently, \(\partial CE/\partial l_i=\hat y_i-\mathbf{1}\{i=c\}\).}
