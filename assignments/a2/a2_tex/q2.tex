    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                \ifans{\(\bm\) is a rolling average of minibatch gradients, so noisy opposite directions tend to cancel while consistent directions remain. The parameter update therefore varies less from step to step than raw SGD. Lower-variance steps reduce oscillation in narrow valleys and make progress along the true descent direction more stable.}
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                \ifans{Parameters with historically smaller gradient magnitudes (small \(v_i\)) get larger updates, because the update is divided by \(\sqrt{v_i}\). This equalizes effective step sizes across coordinates whose gradients live on very different scales: steep or frequently updated parameters are less likely to overshoot, while rarely updated or small-gradient parameters (e.g.\ rare words) can still make meaningful progress.}
            ...
            \subpart[2]
                ...
                \ifans{Each mask entry satisfies \(\mathbb{E}[d_i]=1-p_{\text{drop}}\). Then \(\mathbb{E}[h_{\text{drop},i}]=\gamma h_i\mathbb{E}[d_i]=\gamma(1-p_{\text{drop}})h_i\). Setting this equal to \(h_i\) gives \(\gamma=1/(1-p_{\text{drop}})\).}
          ...
          \ifans{During training, dropout randomly zeros hidden units so neurons cannot co-adapt and the model cannot rely on any particular unit; this regularizes by approximately training an ensemble of thinned subnetworks. At evaluation we want the deterministic full-network prediction, which is the ensemble average. Because \(\gamma\) is chosen so \(\mathbb{E}[\mathbf{h}_{\text{drop}}]=\mathbf{h}\), we simply use \(\mathbf{h}\) with no mask. Applying dropout at test time would inject noise and typically hurt accuracy.}
